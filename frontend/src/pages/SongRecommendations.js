...
function SongRecommendations() {
    ...
    useEffect(() => {
        axios.get("https://flask-9sty.onrender.com/favorites")
            .then((res) => setFavorites(Array.from(new Set(res.data.favorites || []))))
            .catch((err) => console.error("Failed to fetch favorites:", err));
    }, []);
    ...
    const toggleFavorite = async (songTitle) => {
        try {
            const url = "https://flask-9sty.onrender.com/favorites/";
            let res;
            if (favorites.includes(songTitle)) {
                res = await axios.delete(url + "remove", { data: { movie: songTitle } }); // 'movie' key is used by backend
            } else {
    ...
    const handleRecommend = async (songTitle, domain) => {
        if (!songTitle.trim()) return;
        let endpoint = "";
        if (domain === 'similar') endpoint = "https://flask-9sty.onrender.com/crossrecommend/from-song/similar";
        else if (domain === 'books') endpoint = "https://flask-9sty.onrender.com/crossrecommend/from-song/books";
        else if (domain === 'movies') endpoint = "https://flask-9sty.onrender.com/crossrecommend/from-song/movies";
        else return;
